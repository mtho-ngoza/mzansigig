# Privacy Policy

**Last Updated**: [DATE TO BE INSERTED]

**⚠️ IMPORTANT**: This is a POPIA-compliant template. Have it reviewed by a South African lawyer before going live.

---

## 1. Introduction

Kasigig (Pty) Ltd ("Kasigig", "we", "us", or "our") respects your privacy and is committed to protecting your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our platform.

This policy complies with South Africa's **Protection of Personal Information Act (POPIA), Act 4 of 2013**.

By using Kasigig, you consent to the data practices described in this policy.

---

## 2. Information We Collect

### 2.1 Personal Information

We collect the following personal information:

**Account Information**:
- Full name
- Email address
- Phone number
- ID number (for verification)
- Date of birth
- Physical address/location

**Profile Information**:
- Profile photo
- Skills and qualifications
- Work experience
- Portfolio items
- Bio/description
- Preferred work categories

**Verification Information**:
- ID document photos (front and back)
- Selfie photo for identity verification
- Bank account details (for workers receiving payments)

**Payment Information**:
- Payment card details (processed by payment gateway, not stored by us)
- Transaction history
- Earnings and payments

**Usage Information**:
- Gigs posted and applied to
- Messages sent through the platform
- Reviews and ratings
- Device information (IP address, browser, device type)
- Activity logs (login times, pages visited)

### 2.2 Automatically Collected Information

We automatically collect:
- IP address
- Browser type and version
- Device type and operating system
- Pages visited and time spent
- Referring website
- Location data (with permission)

### 2.3 Cookies and Tracking

We use cookies and similar technologies to:
- Keep you logged in
- Remember your preferences
- Analyze platform usage
- Improve user experience

You can control cookies through your browser settings.

---

## 3. How We Use Your Information

### 3.1 Primary Purposes

We use your information to:

**Provide Services**:
- Create and manage your account
- Match workers with gigs
- Facilitate communication between users
- Process payments and transactions
- Provide customer support

**Verification and Safety**:
- Verify your identity
- Prevent fraud and abuse
- Ensure platform safety
- Comply with legal obligations

**Improvement and Analytics**:
- Improve platform features
- Analyze usage patterns
- Develop new features
- Send service updates and notifications

**Marketing** (with consent):
- Send promotional emails
- Notify you of new features
- Share platform news

### 3.2 Legal Basis for Processing (POPIA)

We process your information based on:
- **Consent**: You have given explicit consent
- **Contract**: Necessary to provide our services
- **Legal Obligation**: Required by South African law
- **Legitimate Interest**: For fraud prevention, security, and improvement

---

## 4. Information Sharing and Disclosure

### 4.1 With Other Users

We share limited information with other users:
- **Public Profile**: Name, photo, location, skills, ratings (visible to all users)
- **Application Details**: Shared with employers when you apply to gigs
- **Accepted Gig**: Contact information shared after gig acceptance

### 4.2 With Third-Party Service Providers

We share information with:

**Payment Processors** (PayFast/Yoco):
- Payment card details
- Transaction amounts
- Bank account details (for payouts)

**Cloud Services** (Google Firebase):
- All data stored on Firebase servers
- Subject to Google's security practices

**ID Verification** (Google Cloud Vision API):
- ID document images
- For OCR processing only
- Images processed and stored securely

**Analytics Providers** (if applicable):
- Anonymous usage data
- Device and browser information

### 4.3 Legal Requirements

We may disclose information:
- To comply with legal obligations
- To respond to lawful requests from authorities
- To protect our rights and safety
- To prevent fraud or illegal activity
- In connection with legal proceedings

### 4.4 Business Transfers

If Kasigig is acquired or merged, your information may be transferred to the new entity. You will be notified of any such change.

### 4.5 What We DON'T Do

We will NEVER:
- Sell your personal information to third parties
- Share your data for others' marketing without consent
- Use your ID documents for purposes other than verification

---

## 5. Your Rights Under POPIA

As a South African resident, you have the following rights:

### 5.1 Right to Access

You can request a copy of all personal information we hold about you.

**How**: Email privacy@mzansigig.co.za with subject "Data Access Request"

### 5.2 Right to Correction

You can request correction of inaccurate or incomplete information.

**How**: Update directly in account settings or email us

### 5.3 Right to Deletion

You can request deletion of your personal information (subject to legal retention requirements).

**How**: Email privacy@mzansigig.co.za with subject "Data Deletion Request"

### 5.4 Right to Object

You can object to processing of your data for direct marketing or other purposes.

**How**: Use unsubscribe links in emails or contact us

### 5.5 Right to Restrict Processing

You can request that we limit how we use your data.

**How**: Email privacy@mzansigig.co.za

### 5.6 Right to Data Portability

You can request your data in a machine-readable format.

**How**: Email privacy@mzansigig.co.za with subject "Data Portability Request"

### 5.7 Right to Lodge a Complaint

If you believe we've mishandled your data, you can lodge a complaint with:

**South African Information Regulator**:
- Website: https://www.justice.gov.za/inforeg/
- Email: inforeg@justice.gov.za
- Phone: +27 (0)10 023 5200

---

## 6. Data Security

### 6.1 Security Measures

We protect your information through:

**Technical Measures**:
- Encryption of data in transit (HTTPS/TLS)
- Encryption of sensitive data at rest
- Secure cloud storage (Firebase)
- Regular security audits
- Access controls and authentication

**Organizational Measures**:
- Employee training on data protection
- Limited access to personal information
- Confidentiality agreements
- Incident response procedures

### 6.2 Your Responsibilities

You are responsible for:
- Keeping your password secure
- Not sharing account access
- Reporting suspected security breaches
- Using strong passwords

### 6.3 Data Breach Notification

In the event of a data breach, we will:
- Notify affected users within 72 hours
- Report to the Information Regulator
- Take immediate steps to mitigate harm
- Provide guidance on protective measures

---

## 7. Data Retention

### 7.1 How Long We Keep Data

**Active Accounts**:
- Data retained while account is active
- Plus [X years] after account closure (for legal/accounting purposes)

**ID Verification Documents**:
- Retained for [X years] after account closure
- Or until you request deletion

**Transaction Records**:
- Retained for 7 years (tax and accounting requirements)

**Communication Logs**:
- Retained for [X years] for dispute resolution

### 7.2 Deletion After Retention Period

After retention periods expire, we securely delete data through:
- Overwriting storage
- Permanent deletion from backups
- Destruction of physical copies (if any)

---

## 8. Children's Privacy

Kasigig is not intended for users under 18. We do not knowingly collect information from children. If we discover we've collected a child's information, we will delete it immediately.

If you believe a child has provided information to us, contact: privacy@mzansigig.co.za

---

## 9. International Data Transfers

Your data is primarily stored in:
- **Google Firebase**: Servers in [REGION - e.g., europe-west1]
- **Payment Gateway**: Servers in South Africa

If data is transferred internationally, we ensure adequate protection through:
- Standard contractual clauses
- Adequacy decisions
- Your explicit consent

---

## 10. Cookies Policy

### 10.1 What Are Cookies?

Cookies are small text files stored on your device when you visit our platform.

### 10.2 Types of Cookies We Use

**Essential Cookies** (Required):
- Authentication (keep you logged in)
- Security
- Basic functionality

**Functional Cookies** (Optional):
- Remember preferences
- Improve user experience

**Analytics Cookies** (Optional):
- Track usage patterns
- Improve platform
- Google Analytics (if used)

### 10.3 Managing Cookies

You can control cookies through:
- Browser settings
- Our cookie consent banner
- Opt-out links for analytics

Note: Disabling essential cookies may limit platform functionality.

---

## 11. Marketing Communications

### 11.1 What We Send

With your consent, we may send:
- Platform updates
- New feature announcements
- Tips for using Kasigig
- Promotional offers

### 11.2 Opting Out

You can opt out at any time:
- Click "Unsubscribe" in any email
- Update preferences in account settings
- Email optout@mzansigig.co.za

We will still send essential service emails (password resets, transaction confirmations).

---

## 12. Changes to This Policy

We may update this Privacy Policy to reflect:
- Changes in our practices
- New legal requirements
- Platform improvements

### 12.1 How We Notify You

We will notify you of material changes via:
- Email to registered address
- Platform announcement/banner
- Updated "Last Updated" date

### 12.2 Your Continued Use

Continued use after changes constitutes acceptance of the updated policy.

---

## 13. Contact Us

### 13.1 Privacy Questions

For questions about this policy or your data:

**Email**: privacy@mzansigig.co.za
**Subject Line**: "Privacy Inquiry"

### 13.2 Data Protection Officer

**Name**: [TO BE APPOINTED]
**Email**: dpo@mzansigig.co.za
**Phone**: [TO BE INSERTED]

### 13.3 Company Information

**Kasigig (Pty) Ltd**
**Registration Number**: [TO BE INSERTED]
**Physical Address**: [TO BE INSERTED]
**Postal Address**: [TO BE INSERTED]
**Website**: https://mzansigig.co.za

### 13.4 POPIA Information Officer

As required by POPIA:

**Name**: [TO BE APPOINTED]
**Email**: infofficer@mzansigig.co.za
**Phone**: [TO BE INSERTED]
**Address**: [TO BE INSERTED]

---

## 14. Consent

By using Kasigig, you consent to:
- Collection of information as described
- Use of information for stated purposes
- Sharing with third-party service providers
- Processing in accordance with POPIA

You can withdraw consent at any time by:
- Closing your account
- Contacting privacy@mzansigig.co.za
- Limiting data sharing in settings

---

## 15. Special Provisions

### 15.1 ID Verification Consent

By uploading ID documents, you specifically consent to:
- OCR processing to extract information
- Secure encrypted storage
- Use for verification purposes only
- Retention as per our retention schedule

### 15.2 Location Data

We collect location data only with your permission to:
- Match you with nearby gigs
- Improve search results
- Provide location-based features

You can disable location access in device settings.

---

## Acknowledgment

BY USING KASIGIG, YOU ACKNOWLEDGE THAT YOU HAVE READ AND UNDERSTOOD THIS PRIVACY POLICY AND CONSENT TO THE COLLECTION, USE, AND DISCLOSURE OF YOUR PERSONAL INFORMATION AS DESCRIBED.

---

**Version**: 1.0
**Effective Date**: [TO BE INSERTED]
**POPIA Compliant**: Yes
**Last Reviewed**: [TO BE INSERTED]

---

## Appendix: Glossary of Terms

**Personal Information**: Any information relating to an identifiable person (POPIA definition)

**Processing**: Any operation performed on personal information (collection, storage, use, etc.)

**Data Subject**: The individual whose personal information is being processed (you)

**Responsible Party**: Kasigig (the entity determining the purpose and means of processing)

**Operator**: Third parties processing data on our behalf (payment processors, etc.)

**POPIA**: Protection of Personal Information Act, 2013 (Act No. 4 of 2013)
