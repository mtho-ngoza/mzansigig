...
- [ ] **Simplified application process** - With comprehensive job seeker profiles now viewable via "View Profile" button, consider simplifying the application form. Current form requires cover letter and proposed rate, but employers can already see skills, experience, portfolio, and reviews in the profile. Could streamline to just: proposed rate + optional brief message (instead of full cover letter). This reduces friction for job seekers while employers get better information from the structured profile.
